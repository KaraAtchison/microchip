...
typedef enum {
    EEPROM_CMD_READ  = 0x03, // read memory
    EEPROM_CMD_WRITE = 0x02, // write memory
    EEPROM_CMD_WRDI  = 0x04, // reset the write enable latch (disable writes)
    EEPROM_CMD_WREN  = 0x06, // set the write enable latch (enable writes)
    EEPROM_CMD_RDSR  = 0x05, // read status register
    EEPROM_CMD_WRSR  = 0x01,  // write status register
...
union _MCHP25LC256Status_ DRV_NVM_MCHP25LC256_ReadStatusRegister(void)
{
    ...
    DRV_SPI_Initialize((DRV_SPI_INIT_DATA *)&eepromInitData);
...
******************************************************************************/
uint8_t DRV_NVM_MCHP25LC256_Write(uint32_t address, uint8_t *pData, uint16_t nCount)
{
    uint16_t    addr;
    uint8_t     *pD, temp;
...
/******************************************************************************
  ...
  Returns:
    None
******************************************************************************/
void DRV_NVM_MCHP25LC256_Read(uint32_t address, uint8_t *pData, uint16_t nCount)
{
    uint16_t    addr;
